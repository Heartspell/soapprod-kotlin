...
class AuthRepository(private val pool: Pool) {
    ...
    suspend fun ensureSchema() {
        ...
        pool.query("EXEC sp_SetupAuthProcedures").execute().coAwait()
    }

    suspend fun getRoles(): List<RoleItem> {
        val rows = pool.query("EXEC sp_ListRoles").execute().coAwait()
    ...
    suspend fun getRoleIdByName(name: String): Int? {
        val rows = pool.preparedQuery("EXEC sp_GetRoleIdByName ?").execute(Tuple.of(name)).coAwait()
        val it = rows.iterator()
        return if (it.hasNext()) it.next().requireInt("Id") else null
    }

    suspend fun getUserIdByUsername(username: String): Int? {
        val rows = pool.preparedQuery("EXEC sp_GetUserIdByUsername ?").execute(Tuple.of(username)).coAwait()
        val it = rows.iterator()
        return if (it.hasNext()) it.next().requireInt("Id") else null
    }

    suspend fun saveAdminSeed(hash: ByteArray, salt: ByteArray) {
        pool.preparedQuery("EXEC sp_EnsureAuthSeed ?, ?")
    ...
    suspend fun saveRolePermission(roleId: Int, moduleKey: String, canEdit: Boolean, canDelete: Boolean) {
        pool.preparedQuery("EXEC sp_SaveRolePermissions ?, ?, ?, ?")
    ...
    suspend fun deleteUser(id: Int) {
        pool.preparedQuery("EXEC sp_DeleteUser ?").execute(Tuple.of(id)).coAwait()
    }

    suspend fun getUserRoleIds(userId: Int): List<Int> {
        val rows = pool.preparedQuery("EXEC sp_GetUserRoleIds ?").execute(Tuple.of(userId)).coAwait()
    ...
    suspend fun getUsersWithRoles(): List<UserWithRoles> {
        val rows = pool.query("EXEC sp_ListUsersWithRoles").execute().coAwait()
    ...
    suspend fun getUserAuth(username: String): UserAuthInfo? {
        val rows = pool.preparedQuery("EXEC sp_GetUserAuth ?").execute(Tuple.of(username)).coAwait()
